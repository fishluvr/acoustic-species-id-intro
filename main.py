...
# Helper method to find start hour:
...
# (Row 0 is temporary; will be removed later)
stratified_sample = successful_audiomoth_data.take([0])

# Loop through all 31 AudioMoths
for a in range(31):
    audiomoth_current_code = successful_audiomoth_data[successful_audiomoth_data.get('AudioMothCode') == audiomoth_code_list[a]]

    # 24 clips per AudioMoth (1 per hour)
    for i in range(24):
        audiomoth_data_clips = audiomoth_current_code[audiomoth_current_code.get('Hour') == i].reset_index()
...
# DataFrame returned as CSV file
